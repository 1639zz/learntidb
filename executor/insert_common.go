...
func insertRows(ctx context.Context, base insertCommon) (err error) {
	...
	//如果插入的数据不是常量，调用evalRow函数
	//insertRows在填充函数时，会判断短数据类型，当数据类型不是常量时，将会依赖evalRow进行填充，否则使用fastEvalRow进行填充
